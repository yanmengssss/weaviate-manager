...
export async function GET(req: Request) {
    try {
        ...
        const result = await withRedisClient(config, async (client) => {
            const scanResult = await client.scan(cursorParam, { MATCH: pattern, COUNT: Number(countParam) });
            ...
            const items = keys.map((key, index) => ({
                key,
                type: meta ? (meta[index * 2] as string) : "unknown",
                ttl: meta ? (meta[index * 2 + 1] as number) : -1
            }));
